...
const AddRoom = () => {
  ...
  useEffect(() => {
    const fetchRoom = async () => {
      if (id) {
        try {
          const response = await axios.get(`https://hotel-booking-system1-production.up.railway.app/api/rooms/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (id) {
        await axios.put(`https://hotel-booking-system1-production.up.railway.app/api/rooms/${id}`, room);
        setSnackbarMessage('Room updated successfully!');
      } else {
        await axios.post('https://hotel-booking-system1-production.up.railway.app/api/rooms', room);
